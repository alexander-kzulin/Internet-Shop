...
alter table if exists products_categories
    add constraint
        products_categories_fk_products
...
DROP SEQUENCE IF EXISTS orders_detailsList_seq;
create sequence orders_detailsList_seq start 1 increment 1;

DROP TABLE IF EXISTS orders_detailsList CASCADE;
create table orders_detailsList (
                                    Order_id int8 not null,
                                    detailsList_id int8 not null);


alter table if exists orders_detailsList
    add constraint orders_detailsList_fk
        foreign key (detailsList_id) references orders_details;
alter table if exists orders_detailsList
    add constraint orders_detailsList_fk_products foreign key (Order_id) references orders